...
* Node<K,V>
  ...
  链表元素，实现了 Map.Entry<K,V> 接口

* TreeNode<K,V>

  红黑树元素，继承自 LinkedHashMap.Entry<K,V> 类
...
* 新增操作概述
    1. new HashMap() 为懒加载，并不会直接创建，而是先设置默认加载因子 DEFAULT_LOAD_FACTOR，为0.75；即达到当前容量的 3/4 时，数组会扩容成原来的两倍
    ...
    3. putVal() 方法
        ...
        3. 若当前索引位置不为 null（发生了哈希碰撞），则开始遍历哈希桶当前索引位置上的链表或者直接将元素加入红黑树
        ...
        6. 遍历过程中如果发现当前链表长度大于或者等于 8，则将此链表转换成红黑树（若数组长度小于64，则不会变成红黑树，而是对数组进行扩容，以保证高效率）
* 扩容操作概述
